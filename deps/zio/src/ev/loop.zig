...
const backend_type = @import("backend.zig").backend;
...
pub const LoopState = struct {
    ...
    /// Completions routed from other executors via shared IOCP port.
    /// These need inflight_io decrement when processed (the submitting loop
    /// incremented its counter, but a different loop dequeued the completion).
    routed_completions: AtomicStack(Completion) = .{},
    ...
    /// On IOCP, completions from a shared port may be dequeued by a different
    /// executor than the one that submitted the operation. When this happens,
    /// we route the completion back to the owning loop so the callback runs
    /// on the correct executor thread (preserving coroutine thread affinity).
    pub fn markCompletedFromBackend(self: *LoopState, completion: *Completion) void {
        if (completion.loop) |owning_loop| {
            if (owning_loop != self.loop) {
                // Route to the owning executor's thread
                owning_loop.state.routed_completions.push(completion);
                owning_loop.wake();
                return;
            }
        }
...
pub const Loop = struct {
    ...
    pub fn processCompletions(self: *Loop) void {
        // Process completions routed from other executors (IOCP shared port).
        // These were submitted on our loop (inflight_io++) but dequeued by
        // another executor, which forwarded them here for correct thread affinity.
        var routed = self.state.routed_completions.popAll();
        while (routed.pop()) |completion| {
            self.state.inflight_io -|= 1;
            self.state.markCompleted(completion);
        }
    ...
    pub fn tick(self: *Loop, wait: bool) !void {
        ...
        if (wait) {
            ...
            if (!self.state.completions.empty() or !self.state.work_completions.empty() or !self.state.routed_completions.empty() or timer_result.fired) {
                timeout = .zero;
        ...
        // On IOCP, always poll because completions from the shared port may
        // belong to operations submitted by other executors.
        const should_poll = wait or self.state.inflight_io > 0 or (backend_type == .iocp);
